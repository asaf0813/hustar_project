...
def predict(width, height, confidences, boxes, prob_threshold, iou_threshold=0.5, top_k=-1):
    """
    Select boxes that coos.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'ntain human faces
    Args:
...
print("5")
TRAINING_BASE = 'faces/training/faceimage'
print("6")
...
for label in dirs:
    for i, fn in enumerate(os.listdir(os.path.join(TRAINING_BASE, label))):
        if ('jpg' in fn) or ('png' in fn):
            ...
            raw_img = cv2.imread(os.path.join(TRAINING_BASE, label, fn), cv2.IMREAD_COLOR)
            ...
            if boxes.shape[0] > 0:
                ...
                cv2.imwrite(f'faces/tmp/{label}_{i}.jpg', aligned_face)
